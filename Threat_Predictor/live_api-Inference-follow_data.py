...
def damage_in_window_rs(ros_m_min, asset_value_per_ha, area_km2=5.0):
    """
    Damage (Rs) for fixed area window, scaled by fire intensity
    Higher ROS = more severe fire = higher damage per hectare
    """
    A_ha = area_km2 * 100  # Convert km² to hectares
    
    # Scale damage based on ROS intensity
    # ROS range: 0.01-10 m/min → damage multiplier: 0.1-2.0
    ros_multiplier = min(2.0, max(0.1, 0.1 + (ros_m_min * 0.19)))
    
    # Calculate total damage with ROS-based scaling
    total_damage = A_ha * asset_value_per_ha * ros_multiplier
